...
try:
    ...
    MEDIAN_FILTER_AVAILABLE = True
except ImportError:
    MEDIAN_FILTER_AVAILABLE = False
...
class DetectionSettings:
    ...
    node_dev_threshold: float = -1000.0  # Node deviation threshold (pg) - Set to -1000 to keep all peaks found by threshold
    # R default: tDoubletGap = c(5,250)/20e3 = c(0.00025, 0.0125) seconds
    # Standard Paella default is 15ms
    t_doublet_gap_min: float = 0.00025  # Minimum time between doublet peaks (seconds) - R default: 5/20e3
    t_doublet_gap_max: float = 0.015  # Maximum time between doublet peaks (seconds) - Paella default: 15ms
    max_height_diff_percent: float = 15.0  # Maximum height difference between doublet peaks (percentage) - default: 15%
    max_height_diff_abs: float = 10.0  # Maximum height difference between doublet peaks (Hz) - Paella default: 10Hz
...
class DataStructure:
    ...
    packet_numbers: Optional[np.ndarray] = None  # Packet numbers mapping to frequencies
    relative_times: Optional[np.ndarray] = None  # Relative experiment times
...
class PeakPair:
    ...
    raw_peak1_min: float = 0.0  # Raw minimum frequency at peak 1
    raw_peak2_min: float = 0.0  # Raw minimum frequency at peak 2
    packet_number: int = 0  # Packet number associated with the peak
    relative_time: float = 0.0  # Relative experiment time
...
def estimate_baseline_rolling_median(
    ...
    # Use scipy.ndimage median filter if available (very fast on small numpy arrays)
    if MEDIAN_FILTER_AVAILABLE:
        baseline = median_filter(data, size=window_size_samples, mode='nearest')
        return baseline
    
    # Fallback: use polars rolling median 
    if POLARS_AVAILABLE:
        series = pl.Series(data)
        # Support both older and newer polars signatures
        try:
            baseline = series.rolling_median(window_size=window_size_samples, center=True, min_samples=1).to_numpy()
        except TypeError:
...
def read_single_data(
    ...
    if isinstance(frequency_data, tuple) and len(frequency_data) == 2:
        times, frequencies = frequency_data
        times = np.ascontiguousarray(times, dtype=np.float64)
        frequencies = np.ascontiguousarray(frequencies, dtype=np.float64)
    else:
        # Convert list of tuples to numpy arrays (backward compatibility)
...
def _detect_baseline_and_chunks(
    ...
    # Optimized vectorized approach using convolve for uniform filter
    kernel = np.ones(window_size) / window_size
    baseline_means = signal.convolve(frequencies, kernel, mode='same')
    
    # Calculate rolling std using Var(X) = E[X^2] - E[X]^2
    # E[X^2]
    freq_sq = frequencies**2
    baseline_sq_means = signal.convolve(freq_sq, kernel, mode='same')
    # Var = E[X^2] - E[X]^2
    baseline_vars = np.maximum(0, baseline_sq_means - baseline_means**2)
    baseline_stds = np.sqrt(baseline_vars)
    
    # Overall baseline statistics
    baseline_mean = np.mean(baseline_means)
    baseline_std = np.mean(baseline_stds)
    
    # Detect baseline regions: frequency within threshold_std * std_dev of LOCAL rolling mean
    # (Using local mean allows for baseline drift handling)
    threshold = threshold_std * baseline_std
    baseline_mask = np.abs(frequencies - baseline_means) <= threshold
...
def _find_doublet_pairs(
    ...
    for i in range(len(peak_heights) - 1):
        ...
        # Check height criteria: either percentage OR absolute Hz diff must pass
        # (This unifies R-parity % logic with Paella's absolute Hz logic)
        height_diff_abs = abs(height1 - height2)
        if height_diff_percent > max_height_diff_percent and height_diff_abs > max_height_diff_abs:
            continue
...
def get_all_peaks(
    ...
    Args:
        ...
        suppress_warnings: Whether to suppress diagnostic warnings (e.g., if finding no peaks is expected)
    ...
    if len(indices_below) == 0:
        ...
        if not suppress_warnings and len(detection_data) > 0:
    ...
    doublet_indices = _find_doublet_pairs(
        ...
        detection_settings.max_height_diff_abs
    )
    ...
    if len(peak_pairs) > 0:
        ...
        max_antinode_diff = detection_settings.max_height_diff_abs  # Match Paella's Hz difference limit
